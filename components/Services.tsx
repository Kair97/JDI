...
function ServiceCard({ service, index }: { service: Service; index: number }) {
  ...
  return (
    <Reveal delay={index * 0.07}>
      <article className="group relative h-full overflow-hidden rounded-card bg-white p-6 shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-card-hover sm:p-8">
        {/* тонкая акцентная полоса сверху — как на постерах */}
        <span
